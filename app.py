...
if selected == "Readme":
    ...
    st.write("In conclusion, while both Algorithm-1 and Algorithm-4 guarantee exact solutions to the Densest Subgraph Discovery (DSD) problem, their practical performance and scalability differ significantly. Algorithm-1, although conceptually straightforward, operates on the entire graph during every iteration of its binary search, leading to substantial computational overhead, especially for large and dense graphs. The construction of large flow networks and repeated maximum flow computations make it less suitable for real-world scenarios involving graphs with millions of nodes and edges. On the other hand, Algorithm-4 introduces a crucial optimization by leveraging the inherent structure of the graph through (k, Ψ)-core decomposition. By restricting the densest subgraph search to only the most promising dense regions of the graph, Algorithm-4 dramatically reduces both the size of the flow networks built and the number of computations required. This core-based pruning significantly enhances runtime performance, as observed in experimental evaluations where Algorithm-4 achieved up to four orders of magnitude improvement over Algorithm-1. Furthermore, the tighter initial bounds obtained from the core structure enable faster convergence during binary search, reducing the number of flow computations needed. The additional preprocessing step of core decomposition in Algorithm-4 is highly efficient and is outweighed by the substantial gains in the main computation phase.Overall, while both algorithms maintain correctness and guarantee exact solutions, Algorithm-4 demonstrates vastly superior computational efficiency, scalability, and practical usability. Its ability to process large-scale real-world graphs effectively makes it the recommended approach for DSD tasks in both academic research and industry applications.")